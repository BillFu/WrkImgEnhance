...
void frangi2d_hessian(const Mat &src, Mat &Dxx, Mat &Dxy, Mat &Dyy, float sigma)
{
    ...
    float factor = 3;
    int n_kern_x = 2 * round(factor*sigma) + 1;
    ...
    int i = 0, j = 0;
    for (int x = -round(factor*sigma); x <= round(factor*sigma); x++)
    {
        j = 0;
        for (int y = -round(factor*sigma); y <= round(factor*sigma); y++)
        {
...
void frangi2d_createopts_pore(frangi2d_opts *opts) {
    //these parameters depend on the scale of the vessel, depending ultimately on the image size...
    opts->sigma_start = DEFAULT_SIGMA_START_PORE;
    opts->sigma_end = DEFAULT_SIGMA_END_PORE;
    opts->sigma_step = DEFAULT_SIGMA_STEP;
    
    opts->BetaOne = DEFAULT_BETA_ONE_PORE; //ignore blob-like structures?
    opts->BetaTwo = DEFAULT_BETA_TWO_PORE; //appropriate background suppression for this specific image, but can change.
...
void frangi2d_createopts_seg(frangi2d_opts *opts) {
    ...
    opts->BetaOne = DEFAULT_BETA_ONE_SEG; //ignore blob-like structures?
    opts->BetaTwo = DEFAULT_BETA_TWO_SEG; //appropriate background suppression for this specific image, but can change.
...
void frangi2d(const Mat &src, Mat &maxVals, Mat &whatScale, Mat &outAngles, frangi2d_opts opts)
{
    vector<Mat> ALLfiltered;
    vector<Mat> ALLangles;
    float beta = 2.0 * opts.BetaOne*opts.BetaOne;
    float c = 2.0 * opts.BetaTwo*opts.BetaTwo;
